...
module.exports = srv => {
    const { MultistepLog, MultistepHeadLog } = srv.entities;
    ...
    // 在头表 MultistepHeadLog 上注册重推 action
    srv.on('retryStep', MultistepHeadLog, async (req) => {
        const targetKey = req.params[0];
        if (!targetKey) return req.error(400, '请选择数据');

        const zrfcLogid = targetKey.zrfc_logid;

        console.log('[MultistepLogService] 收到重推请求 - zrfcLogid:', zrfcLogid);
        ...
        try {
            ...
            return SELECT.one(MultistepHeadLog).where(targetKey);
