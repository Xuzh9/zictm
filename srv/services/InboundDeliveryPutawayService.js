    ...
    class InboundDeliveryPutawayService {
        ...
        async execute(inputData) {
            try {
                ...
                console.log('[InboundDeliveryPutawayService] 销售订单类型:', salesOrderType);

                // 借贷项订单（CR/DR）不需要交货单操作，直接跳过
                if (salesOrderType === 'CR' || salesOrderType === 'DR') {
                    console.log(`[InboundDeliveryPutawayService] 销售订单类型 ${salesOrderType} 为借贷项订单，步骤跳过`);
                    const returnResult = {
                        code: 'S',
                        message: `销售订单类型 ${salesOrderType} 为借贷项订单，跳过内向交货单更新入库数量`,
                        objkey: ''
                    };
                    console.log('[InboundDeliveryPutawayService] 返回结果:', JSON.stringify(returnResult));
                    return returnResult;
